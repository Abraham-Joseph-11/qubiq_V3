...
arduinoGenerator.forBlock['esp32_analog_read'] = function (block) {
    ...
    // Map text values to actual analog pins. LDR is defined as pin 34 in some configs, or 36.
    // Ensure "IN1" etc. are mapped to valid ADC pins. Let's use 34 for LDR as a fallback.
    // If it's a number, it will just pass through.
    if (pin === 'LDR') pin = '34';
    else if (pin === 'IN1') pin = '34';
    else if (pin === 'IN2') pin = '35';
    else if (pin === 'IN4') pin = '32';
    else if (pin === 'IN5') pin = '33';
...
// 1.5 If/Else
arduinoGenerator.forBlock['custom_controls_ifelse'] = function (block) {
    var conditionCode = arduinoGenerator.valueToCode(block, 'IF0', arduinoGenerator.ORDER_NONE) || 'false';
    var branchIfCode = arduinoGenerator.statementToCode(block, 'DO0');
    var branchElseCode = arduinoGenerator.statementToCode(block, 'ELSE');
    return '  if (' + conditionCode + ') {\n' + branchIfCode + '  } else {\n' + branchElseCode + '  }\n';
};

// 1.6 If/Else-If
arduinoGenerator.forBlock['custom_controls_if_ifnot'] = function (block) {
    var conditionIfCode = arduinoGenerator.valueToCode(block, 'IF0', arduinoGenerator.ORDER_NONE) || 'false';
    var branchIfCode = arduinoGenerator.statementToCode(block, 'DO0');
    var conditionElseIfCode = arduinoGenerator.valueToCode(block, 'IF1', arduinoGenerator.ORDER_NONE) || 'false';
    var branchElseIfCode = arduinoGenerator.statementToCode(block, 'DO1');
